...
#### Networking
- > A Summary of what networking is: Hosts on the Ethernet (Local Network) Basically send out Recursive Ethernet Packets on Protocol ARP(address resolution display and control) Which Requests to resolve an IP Adress into it's connected Interface Mac Adress. The gateway / Router's task is to recursively send and read the payload as the defined protocol to read both the destination and the required information In-Depth Explanation: https://www.youtube.com/watch?v=xNbdeyEI-nE.
