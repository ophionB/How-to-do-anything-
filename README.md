...
#### Networking
- > A Summary of what networking is: Hosts on the ethernet (Local Network) sending out recursive ARP(address resolution protocol) requests. An ARP packet requests to resolve an IP Adress to an interfaces Mac Adress(Where is 192.168.xx.xx? Tell 192.168.source.ip). The gateway / Router's task is to recursively request and reply until the destined IP Adress has been reached and replied to our request. The IP(Internet Protocol)'s Frame is located entirely within the ARP's payload. In-Depth Explanation: https://www.youtube.com/watch?v=xNbdeyEI-nE.
